...
public class FirstPersonController : MonoBehaviour
{
    ...
    public float interactRange = 2f; // Distance for interaction
    ...
    private void HeadBob()
    {
     ...
     private void CheckForInteraction(){
        // Create a ray starting from the player position forward
        Ray ray = new Ray(transform.position, transform.forward);
        if (Physics.Raycast(ray, out RaycastHit hit, interactRange))
        {
            // Check if the object has the Interactable tag and IClickable component
            if (hit.collider.CompareTag("Interactable"))
            {
                // Display UI or feedback if within range
